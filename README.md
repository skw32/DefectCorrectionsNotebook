...
This project is currently under development. Tests are currently being run to determine sensible default parameters (although there will later always be the option for manual override by the user). The project is currently only able to perform corrections for intrinsic point defects. Please see the 'Assumptions' section of the notebook for assumptions made about the defects data during the processing performed by the notebook.
